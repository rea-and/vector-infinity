...
if [ "$SKIP_SSL" = true ]; then
    echo "SSL certificate setup will be skipped (-nocert)"
fi
echo ""
...
if [ "$SKIP_SSL" = true ]; then
    echo ""
    echo "Step 13: Skipping SSL certificate setup (-nocert option used)"
...
echo "   - For Gmail plugin, add credentials.json file"
echo "   - For WhatsApp plugin, upload chat export ZIP files via the web UI"
echo ""
echo "2. Start the application:"
echo "   Option A - Use the launcher script (recommended):"
echo "   ./start.sh"
echo ""
...
echo ""
echo "3. To install as a systemd service (optional, for production):"
